...
const services = [
  {
    ...
    features: [
      ...
      "Tires scrubbed clean then dressed with a UV-protective coating — stops rubber from cracking, fading, and going grey",
      ...
      "* Additional fee applies for pet hair removal",
    ],
    highlight: false,
  },
  {
  ...
  },
  {
    ...
    features: [
      "Everything in the exterior detail — full wash, brake dust removal, UV-protective tire dressing, paint protection sealant",
                ...
                <div className="p-7 lg:p-8 flex flex-col h-full">
                  ...
                  <ul className="space-y-2.5 mb-8 flex-1">
                    {service.features.map((feature) => {
                      const isNote = feature.startsWith("*");
                      return (
                        <li key={feature} className={`flex items-start gap-3 text-sm ${isNote ? "mt-4 pt-4 border-t border-current/10" : ""}`}>
                          {isNote ? (
                            <span className={`text-xs leading-relaxed italic ${service.highlight ? "text-cream/40" : "text-charcoal-400"}`}>
                              {feature}
                            </span>
                          ) : (
                            <>
